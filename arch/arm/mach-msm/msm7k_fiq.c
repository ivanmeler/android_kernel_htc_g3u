...
void msm7k_fiq_handler(void)
{
	...
	char *shmem_radio_err_msg = (char *)(base+0xE0000);
	char radio_err_msg[SZ_DIAG_ERR_MSG+1];
	...
	fiq_counter += 1;
	if(this_cpu == 0) {
		memset(radio_err_msg, 0, sizeof(radio_err_msg));
		memcpy(radio_err_msg, shmem_radio_err_msg, sizeof(radio_err_msg)-1);
		printk("radio_err_msg=%s\n", radio_err_msg);
	}
