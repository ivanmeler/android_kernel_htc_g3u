...
struct audio {
	...
	#ifdef CONFIG_PERFLOCK
	struct perf_lock audio_perf_lock;
	#endif
...
static int audio_enable(struct audio *audio)
{
	...
	#ifdef CONFIG_PERFLOCK
	perf_lock(&audio->audio_perf_lock);
	#endif
...
static int audio_disable(struct audio *audio)
{
	...
	if (audio->enabled) {
		...
		#ifdef CONFIG_PERFLOCK
		perf_unlock(&audio->audio_perf_lock);
		#endif
	}
	return 0;
}
...
static int __init audio_init(void)
{
	...
	#ifdef CONFIG_PERFLOCK
	perf_lock_init(&the_audio.audio_perf_lock, TYPE_PERF_LOCK, PERF_LOCK_LOWEST, "audio_pcm");
	#endif
