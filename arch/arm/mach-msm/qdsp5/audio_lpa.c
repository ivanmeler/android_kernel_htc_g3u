...
static void get_current_time_ns(s64* ns_time)
{
	struct timeval curr_time;
	do_gettimeofday(&curr_time);
	if (ns_time != NULL) {
		*ns_time = timeval_to_ns(&curr_time);
	}
}
...
static long audio_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case AUDIO_PAUSE: {
		bool skip_ramping = false;
		s64 curr_time_ns;
		get_current_time_ns(&curr_time_ns);

		MM_DBG("curr - prev = %lld", curr_time_ns - audio->prev_time);
		if (curr_time_ns - audio->prev_time < 1000000000) {
			skip_ramping = true;
			audio->want_ramping = 2;
		}
