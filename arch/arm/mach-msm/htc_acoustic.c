...
#if defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_Z3DUG) || defined(CONFIG_MACH_Z3DCG) || defined(CONFIG_MACH_G3U)
...
#define ONCRPC_ENABLE_EXTMOD_SPK_PROC	(17)
			   ...
			   unsigned long arg)
{
	...
	case ACOUSTIC_SW_SPKL_RECVR:
		...
		pr_aud_info("htc_acoustic switch_spkl_recvr status: %s\n", cpld_gpio_read(CPLD_EXT_GPIO_AUD_SPK_RCV_SEL)?"Receiver" : "Speaker");
		rc = 0;
		break;
	case ACOUSTIC_ENABLE_RECVR:
		if (copy_from_user(&dev_en, (void *)arg, sizeof(dev_en))) {
			rc = -EFAULT;
			break;
		}
		pr_aud_info("htc_acoustic enable_recvr: %s\n", dev_en ? "Receiver Enable" : "Receiver Disable");
		if (dev_en < 0 || dev_en > 1) {
			pr_aud_err("Invalid parameter of receiver enable");
			rc = -EINVAL;
			break;
		}
		cpld_gpio_write(CPLD_EXT_GPIO_AUD_REV_EN, dev_en);
		rc = 0;
		break;
	...
	case ACOUSTIC_ENABLE_EXTMOD_SPK:
		...
		extmodspk_req.enable = cpu_to_be32(extspk_arg);
		pr_aud_err("htc_acoustic enable_ext_modem_speaker: %d\n", extspk_arg);
		rc = msm_rpc_call(endpoint, ONCRPC_ENABLE_EXTMOD_SPK_PROC,
			&extmodspk_req, sizeof(extmodspk_req), 5*HZ);
		if (rc < 0)
	...
	case ACOUSTIC_SET_AMP:
		...
		pr_aud_info("htc_acoustic set 9887 amp: %s\n", setamp_arg ? "Enable" : "Disable");
		if (setamp_arg < 0 || setamp_arg > 1) {
			pr_aud_err("Invalid parameter of set 9887 amp");
			rc = -EINVAL;
			break;
		}
		set_tfa9887_spkamp(setamp_arg, 0);
#ifdef CONFIG_AMP_TFA9887L
		set_tfa9887l_spkamp(setamp_arg,0);
#endif
		rc = 0;
		break;
#endif
	...
	case ACOUSTIC_GET_PCBID:
		...
		if (copy_to_user((void *) arg,
			&htc_pcbid, sizeof(htc_pcbid))) {
			pr_aud_err("acoustic_ioctl: "
				"ACOUSTIC_GET_PCBID failed\n");
			rc = -EFAULT;
		} else
			rc = 0;
