/* linux/arch/arm/mach-msm/board-g3-panel.c
 *
...
static struct dsi_buf g3_panel_tx_buf;
static struct dsi_buf g3_panel_rx_buf;
...
static void g3_panel_power(int on)
{
	int rc;
	...
	if (on) {
		...
		if (panel_type == PANEL_ID_NAIAD_AUO_NT ||
			panel_type == PANEL_ID_NAIAD_AUO_NT_C2) {
			gpio_set_value(LCMIO_1V8_EN, 1);
			hr_msleep(1);
			gpio_set_value(LCM_3V_EN, 1);
		} else if (panel_type == PANEL_ID_NAIAD_JDI_NT) {
			gpio_set_value(LCMIO_1V8_EN, 1);
			hr_msleep(10);
			gpio_set_value(LCM_3V_EN, 1);
		}
		...
		if (panel_type == PANEL_ID_NAIAD_AUO_NT ||
			panel_type == PANEL_ID_NAIAD_AUO_NT_C2) {
			gpio_set_value(LCM_RSTz, 0);
			hr_msleep(1);
			gpio_set_value(LCM_3V_EN, 0);
			hr_msleep(5);
			gpio_set_value(LCMIO_1V8_EN, 0);
		} else if (panel_type == PANEL_ID_NAIAD_JDI_NT) {
			hr_msleep(10);
			gpio_set_value(LCM_RSTz, 0);
			hr_msleep(10);
...
static unsigned char g3_shrink_pwm(int val)
{
...
static int g3_lcd_on(struct platform_device *pdev)
{
	...
	if (mfd->first_init_lcd != 0) {
		...
		if (panel_type != PANEL_ID_NONE) {
			...
			mipi_dsi_cmds_tx(&g3_panel_tx_buf, power_on_cmd,
				power_on_cmd_count);
...
static int g3_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
...
static void g3_dim_on(struct msm_fb_data_type *mfd)
{
        mipi_dsi_cmds_tx(&g3_panel_tx_buf, dim_on_cmd, 1);
        PR_DISP_INFO("%s\n", __func__);
}
static void g3_dim_off(struct msm_fb_data_type *mfd)
{
        mipi_dsi_cmds_tx(&g3_panel_tx_buf, dim_off_cmd, 1);
        PR_DISP_INFO("%s\n", __func__);
}
...
static void g3_set_cabc_lvl(struct msm_fb_data_type *mfd, int mode)
{
	...
	mipi_dsi_cmds_tx(&g3_panel_tx_buf, cabc_cmds, 2);
}
#endif

static void g3_set_backlight(struct msm_fb_data_type *mfd)
{
	PR_DISP_INFO("%s\n", __func__);
	...
	led_pwm1[1] = g3_shrink_pwm(mfd->bl_level);
	...
	mipi_dsi_cmds_tx(&g3_panel_tx_buf, backlight_cmds, 1);
	htc_mdp_sem_up(&mfd->dma->mutex);
...
static void g3_bkl_switch(struct msm_fb_data_type *mfd, bool on)
{
	PR_DISP_INFO("%s > %d\n", __func__, on);
	if (on) {
		mipi_status = 1;
		...
		if (bl_level_prevset != 0) {
			...
			mfd->bl_level = 0;
			g3_set_backlight(mfd);
		}
		mipi_status = 0;
	}
}

static void g3_display_on(struct msm_fb_data_type *mfd)
{
#if 0
...
static void g3_display_off(struct msm_fb_data_type *mfd)
{
#if 0
	...
	mipi_dsi_cmdlist_put(&cmdreq);
#endif
	mipi_dsi_cmds_tx(&g3_panel_tx_buf, display_off_cmd, display_off_cmd_count);
	PR_DISP_INFO("%s\n", __func__);
}

static int __devinit g3_lcd_probe(struct platform_device *pdev)
{
...
static void g3_lcd_shutdown(struct platform_device *pdev)
{
	mipi_panel_power(0);
}
...
static struct platform_driver this_driver = {
	.probe = g3_lcd_probe,
	.shutdown = g3_lcd_shutdown,
	.driver = {
		.name   = "mipi_g3",
	},
};

static struct msm_fb_panel_data g3_panel_data = {
	.on		= g3_lcd_on,
	.off		= g3_lcd_off,
	.set_backlight  = g3_set_backlight,
	.display_on     = g3_display_on,
	.display_off    = g3_display_off,
	.bklswitch      = g3_bkl_switch,
	.dimming_on     = g3_dim_on,
#ifdef CONFIG_FB_MSM_CABC_LEVEL_CONTROL
	.set_cabc       = g3_set_cabc_lvl,
#endif
};

static int ch_used[3];

int mipi_g3_device_register(struct msm_panel_info *pinfo,
					u32 channel, u32 panel)
{
	...
	pdev = platform_device_alloc("mipi_g3", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;

	g3_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &g3_panel_data,
		sizeof(g3_panel_data));
	if (ret) {
...
static int __init mipi_jdi_nt_init(void)
{
	...
	pinfo.wait_cycle = 0;
	pinfo.bpp = 24;
	if(panel_type == PANEL_ID_NAIAD_JDI_NT) {
	...
	ret = mipi_g3_device_register(&pinfo, MIPI_DSI_PRIM,
						MIPI_DSI_PANEL_QHD_PT);

	if (ret)
		PR_DISP_ERR("%s: failed to register device!\n", __func__);

	if(panel_type == PANEL_ID_NAIAD_AUO_NT_C2) {
		...
		strcat(ptype, "PANEL_ID_NAIAD_JDI_NT");
...
static int __init g3_panel_panel_init(void)
{
	mipi_dsi_buf_alloc(&g3_panel_tx_buf, DSI_BUF_SIZE);
	mipi_dsi_buf_alloc(&g3_panel_rx_buf, DSI_BUF_SIZE);

	PR_DISP_INFO("%s: enter 0x%x\n", __func__, panel_type);

	if(panel_type == PANEL_ID_NAIAD_AUO_NT_C2) {
		mipi_jdi_nt_init();
		PR_DISP_INFO("match PANEL_ID_NAIAD_AUO_NT_C2 panel_type\n");
	} else if(panel_type == PANEL_ID_NAIAD_JDI_NT) {
		mipi_jdi_nt_init();
		PR_DISP_INFO("match PANEL_ID_NAIAD_JDI_NT panel_type\n");
	} else if(panel_type == PANEL_ID_NAIAD_AUO_NT) {
		mipi_jdi_nt_init();
		PR_DISP_INFO("match PANEL_ID_NAIAD_AUO_NT panel_type\n");
	} else
...
device_initcall_sync(g3_panel_panel_init);
