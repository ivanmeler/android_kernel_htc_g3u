...
int headset_notifier_register(struct headset_notifier *notifier)
{
	...
	case HEADSET_REG_PMIC_SET_BTN_IRQ_MODE:
		HS_LOG("Register PMIC_SET_BTN_IRQ_MODE notifier");
...
static void mic_detect_work_func(struct work_struct *work)
{
	...
	if (hi->driver_one_wire_exist && hi->one_wire_mode == 0 &&
		((get_kernel_flag() & BIT(1)) == 0) && (board_mfg_mode() == 0)) {
		...
		if (adc > 915) {
			...
			if (hs_mgr_notifier.hs_1wire_init() == 0) {
				hi->one_wire_mode = 1;
#if defined (CONFIG_HTC_HEADSET_PMIC_EDGE_BTN)
				if (hs_mgr_notifier.hs_pmic_set_btn_irq_mode)
					hs_mgr_notifier.hs_pmic_set_btn_irq_mode(HS_PMIC_BTN_IRQ_EDGE_MODE);
#endif
...
#if defined (CONFIG_HTC_HEADSET_PMIC_EDGE_BTN)
				if (hs_mgr_notifier.hs_pmic_set_btn_irq_mode)
					hs_mgr_notifier.hs_pmic_set_btn_irq_mode(HS_PMIC_BTN_IRQ_LEVEL_MODE);
#endif
			}
		}
	}
...
#if defined(CONFIG_CPLD)
	insert = cpld_gpio_read(CPLD_EXT_GPIO_AUD_HP_IN_INPUT_LEVEL);
	if (insert < 0 || insert == 0) {
		HS_LOG("Headset has been inserted - cpld: %d", insert);
		return;
	}
#endif
...
static void insert_detect_work_func(struct work_struct *work)
{
	...
	if (insert < 0 || insert == 1) {
		HS_LOG("Headset has been removed - cpld: %d", insert);
		return;
	}
#endif
	...
	set_35mm_hw_state(1);
	msleep(250); 
	if ((hi->driver_one_wire_exist)&&((get_kernel_flag() & BIT(1)) ==0)
		&& (board_mfg_mode() == 0))
	{
