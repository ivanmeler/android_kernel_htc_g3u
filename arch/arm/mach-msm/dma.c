...
void msm_dmov_channel_dump(int id)
{
	int adm = DMOV_ID_TO_ADM(id);
	int ch = DMOV_ID_TO_CHAN(id);

	pr_info("Flush0 =%x\n", readl_relaxed(DMOV_REG(DMOV_FLUSH0(ch), adm)));
	pr_info("Flush1 =%x\n", readl_relaxed(DMOV_REG(DMOV_FLUSH1(ch), adm)));
	pr_info("Flush3 =%x\n", readl_relaxed(DMOV_REG(DMOV_FLUSH3(ch), adm)));
	pr_info("Flush4 =%x\n", readl_relaxed(DMOV_REG(DMOV_FLUSH4(ch), adm)));
	pr_info("Flush5 =%x\n", readl_relaxed(DMOV_REG(DMOV_FLUSH5(ch), adm)));
	pr_info("CMD_PTR =%x\n", readl_relaxed(DMOV_REG(DMOV_CMD_PTR(ch), adm)));

	pr_info("CH_STATUS =%x\n", readl_relaxed(DMOV_REG(DMOV_STATUS(ch), adm)));
}
EXPORT_SYMBOL(msm_dmov_channel_dump);
	...
	while (int_status) {
		...
		do {
			...
			if (ch_result & DMOV_RSLT_FLUSH) {
				...
				PRINT_IO("msm_datamover_irq_handler id %d, status %x result %x\n", id, ch_status, ch_result);
