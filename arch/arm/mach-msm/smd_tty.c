		    ...
		    unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		case SIOCREADSMD:
			printk(KERN_INFO "%s Read smd channel manually! \n", __FUNCTION__);
			smd_tty_notify(info, SMD_EVENT_DATA);
			ret = 0;
			break;
		case SIOCLASTAT:
			if ( (char *)arg!=NULL && strncpy_from_user(textbuf, (char *)arg, 255)) {
				textbuf[255] = '\0';
				printk(KERN_INFO "%s : timeout AT CMD is %s ! \n", __FUNCTION__, textbuf);
			} else {
				printk(KERN_INFO "%s : can not get AT CMD ! \n", __FUNCTION__);
			}
			ret = 0;
			break;
		default:
			printk(KERN_ERR "%s (%x)not support ioctl! \n", __FUNCTION__, cmd);
			ret = -1;
			break;
	}
...
#if defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_DUMMY) || defined(CONFIG_MACH_G3U)
