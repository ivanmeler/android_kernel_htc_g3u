...
extern void gps_1530_1v8_on(void);
extern void gps_1530_uart_on(int on);
#ifdef CONFIG_GPS_1530_LNA_POWER
extern int gps_1530_lna_power_enable(int on);
#endif
#endif
			    ...
			    struct ktermios *old)
{
...
#ifdef CONFIG_GPS_1530
	if ((termios->c_cflag & CRTSCTS) || (pdev->id == 2)){
#else
	if (termios->c_cflag & CRTSCTS) {
		...
		mr |= UART_MR1_RX_RDY_CTL;
#ifdef CONFIG_GPS_1530
		printk(KERN_INFO "msm_serial: hardware flow control was enabled, port = %d, mr= %x", pdev->id, mr);
#endif
	}
	msm_write(port, mr, UART_MR1);
