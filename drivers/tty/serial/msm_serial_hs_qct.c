...
void qct_msm_hs_request_clock_off(struct uart_port *uport);
			 ...
			 const char *buf, size_t count)
{
	...
	switch (state) {
	case 0: {
		qct_msm_hs_request_clock_off(&msm_uport->uport);
		break;
	}
	case 1: {
		qct_msm_hs_request_clock_on(&msm_uport->uport);
		break;
	}
	default: {
		return -EINVAL;
...
unsigned int qct_msm_hs_tx_empty(struct uart_port *uport)
{
	unsigned int data;
	unsigned int ret = 0;
...
static int msm_hs_check_clock_off(struct uart_port *uport)
{
	...
	pr_info("%s(): clk_state:%x clk_req_off_state:%x"
		"rx.flush:%x\n", __func__, msm_uport->clk_state,
		msm_uport->clk_req_off_state, msm_uport->rx.flush);
...
EXPORT_SYMBOL(qct_msm_hs_request_clock_off);

void qct_msm_hs_request_clock_on(struct uart_port *uport)
{
	struct msm_hs_port *msm_uport = UARTDM_TO_MSM(uport);
	unsigned long flags;
	unsigned int data;
	...
	pr_info("%s(): UART CLOCK ON: clk_state:%x clk_req_off_state:%x"
		"rx.flush:%x\n", __func__, msm_uport->clk_state,
		msm_uport->clk_req_off_state, msm_uport->rx.flush);

	switch (msm_uport->clk_state) {
	case MSM_HS_CLK_OFF:
		printk(KERN_INFO "[BT]R CLK ON\n");
	...
	pr_info("%s(): Exit: clk_state:%x clk_req_off_state:%x"
		"rx.flush:%x\n", __func__, msm_uport->clk_state,
		msm_uport->clk_req_off_state, msm_uport->rx.flush);
	spin_unlock_irqrestore(&uport->lock, flags);
	mutex_unlock(&msm_uport->clk_mutex);
}
EXPORT_SYMBOL(qct_msm_hs_request_clock_on);
...
static irqreturn_t msm_hs_wakeup_isr(int irq, void *dev)
{
	...
	if (wakeup) {
		spin_unlock_irqrestore(&uport->lock, flags);
		qct_msm_hs_request_clock_on(uport);
		spin_lock_irqsave(&uport->lock, flags);
...
static int uartdm_init_port(struct uart_port *uport)
{
	...
	wake_lock_init(&rx->wake_lock, WAKE_LOCK_SUSPEND, "msm_serial_hs_qct_rx");
	wake_lock_init(&msm_uport->dma_wake_lock, WAKE_LOCK_SUSPEND,
		       "msm_serial_hs_qct_dma");
