		...
		void (*getevent)(void *ptr, size_t len))
{
	...
	if (MSG_TABLE_CMD_ACK == id) {
		spin_lock_dbg[SPIN_LOCK_DBG_MSG_TABLE_CMD_ACK_0] = SPIN_LOCK_ENTER;
		spin_lock_irqsave(&vfe2x_ctrl->table_lock, flags);
		spin_lock_dbg[SPIN_LOCK_DBG_MSG_TABLE_CMD_ACK_0] = SPIN_LOCK_EXIT;
		...
		if (list_empty(&vfe2x_ctrl->table_q)) {
			...
			spin_lock_dbg[SPIN_LOCK_DBG_MSG_TABLE_CMD_ACK_1] = SPIN_LOCK_ENTER;
			spin_unlock_irqrestore(&vfe2x_ctrl->table_lock, flags);
			spin_lock_dbg[SPIN_LOCK_DBG_MSG_TABLE_CMD_ACK_1] = SPIN_LOCK_EXIT;
			kfree(data);
			return;
		}
		...
		if (!table_pending) {
			spin_lock_dbg[SPIN_LOCK_DBG_MSG_TABLE_CMD_ACK_2] = SPIN_LOCK_ENTER;
			spin_unlock_irqrestore(&vfe2x_ctrl->table_lock, flags);
			spin_lock_dbg[SPIN_LOCK_DBG_MSG_TABLE_CMD_ACK_2] = SPIN_LOCK_EXIT;
			kfree(data);
			return;
		}
		...
		spin_lock_dbg[SPIN_LOCK_DBG_MSG_TABLE_CMD_ACK_3] = SPIN_LOCK_ENTER;
		spin_unlock_irqrestore(&vfe2x_ctrl->table_lock, flags);
		spin_lock_dbg[SPIN_LOCK_DBG_MSG_TABLE_CMD_ACK_3] = SPIN_LOCK_EXIT;
		...
		*(uint32_t *)cmd_data = header;
		if (queue == QDSP_CMDQUEUE) {
			...
			case VFE_CMD_ZSL:
				pr_info("CMD_GENERAL: before %s %s\n", cmds_map[vfecmd.id].isp_id_name,
					cmds_map[vfecmd.id].vfe_id_name);
				spin_lock_dbg[SPIN_LOCK_DBG_VFE_START_0] = SPIN_LOCK_ENTER;
				spin_lock_irqsave(&vfe2x_ctrl->table_lock, flags);
				spin_lock_dbg[SPIN_LOCK_DBG_VFE_START_0] = SPIN_LOCK_EXIT;
				if ((!list_empty(&vfe2x_ctrl->table_q)) ||
						vfe2x_ctrl->tableack_pending) {
					CDBG("start pending\n");
					vfe2x_ctrl->start_pending = 1;
					spin_lock_dbg[SPIN_LOCK_DBG_VFE_START_1] = SPIN_LOCK_ENTER;
					spin_unlock_irqrestore(
						&vfe2x_ctrl->table_lock,
								flags);
					spin_lock_dbg[SPIN_LOCK_DBG_VFE_START_1] = SPIN_LOCK_EXIT;
					return 0;
				}
				spin_lock_dbg[SPIN_LOCK_DBG_VFE_START_2] = SPIN_LOCK_ENTER;
				spin_unlock_irqrestore(&vfe2x_ctrl->table_lock, flags);
				spin_lock_dbg[SPIN_LOCK_DBG_VFE_START_2] = SPIN_LOCK_EXIT;
			...
			case VFE_CMD_STOP:
				pr_info("CMD_GENERAL: %s %s\n", cmds_map[vfecmd.id].isp_id_name,
					cmds_map[vfecmd.id].vfe_id_name);
				vfestopped = 1;
				spin_lock_dbg[SPIN_LOCK_DBG_VFE_STOP_0] = SPIN_LOCK_ENTER;
				spin_lock_irqsave(&vfe2x_ctrl->table_lock, flags);
				spin_lock_dbg[SPIN_LOCK_DBG_VFE_STOP_0] = SPIN_LOCK_EXIT;
				...
				if ((!list_empty(&vfe2x_ctrl->table_q)) ||
					...
					CDBG("stop pending\n");
					vfe2x_ctrl->stop_pending = 1;
					spin_lock_dbg[SPIN_LOCK_DBG_VFE_STOP_1] = SPIN_LOCK_ENTER;
					spin_unlock_irqrestore(
							&vfe2x_ctrl->table_lock,
							flags);
					spin_lock_dbg[SPIN_LOCK_DBG_VFE_STOP_1] = SPIN_LOCK_EXIT;
					return 0;
				}
				vfe2x_ctrl->snapshot_done = 0;
				spin_lock_dbg[SPIN_LOCK_DBG_VFE_STOP_2] = SPIN_LOCK_ENTER;
				spin_unlock_irqrestore(&vfe2x_ctrl->table_lock,
						flags);
				spin_lock_dbg[SPIN_LOCK_DBG_VFE_STOP_2] = SPIN_LOCK_EXIT;
				vfe2x_ctrl->vfe_started = 0;
				goto config_send;
			case VFE_CMD_UPDATE:
				spin_lock_dbg[SPIN_LOCK_DBG_VFE_UPDATE_0] = SPIN_LOCK_ENTER;
				spin_lock_irqsave(&vfe2x_ctrl->table_lock, flags);
				spin_lock_dbg[SPIN_LOCK_DBG_VFE_UPDATE_0] = SPIN_LOCK_EXIT;
				if ((!list_empty(&vfe2x_ctrl->table_q)) ||
						vfe2x_ctrl->tableack_pending) {
					CDBG("update pending\n");
					vfe2x_ctrl->update_pending = 0;
					vfe2x_send_isp_msg(vfe2x_ctrl,
						...
						isp_id);
					spin_lock_dbg[SPIN_LOCK_DBG_VFE_UPDATE_1] = SPIN_LOCK_ENTER;
					spin_unlock_irqrestore(
							&vfe2x_ctrl->table_lock,
							flags);
					spin_lock_dbg[SPIN_LOCK_DBG_VFE_UPDATE_1] = SPIN_LOCK_EXIT;
					return 0;
				}
				spin_lock_dbg[SPIN_LOCK_DBG_VFE_UPDATE_2] = SPIN_LOCK_ENTER;
				spin_unlock_irqrestore(&vfe2x_ctrl->table_lock, flags);
				spin_lock_dbg[SPIN_LOCK_DBG_VFE_UPDATE_2] = SPIN_LOCK_EXIT;
				goto config_send;
			default:
				break;
			}
...
config_send:
	...
	spin_lock_dbg[SPIN_LOCK_DBG_VFE_CONFIG_SEND_0] = SPIN_LOCK_ENTER;
	spin_lock_irqsave(&vfe2x_ctrl->table_lock, flags);
	spin_lock_dbg[SPIN_LOCK_DBG_VFE_CONFIG_SEND_0] = SPIN_LOCK_EXIT;
	if (queue == QDSP_TABLEQUEUE &&
		...
		if (!table_pending) {
			rc = -ENOMEM;
			spin_lock_dbg[SPIN_LOCK_DBG_VFE_CONFIG_SEND_1] = SPIN_LOCK_ENTER;
			spin_unlock_irqrestore(&vfe2x_ctrl->table_lock, flags);
			spin_lock_dbg[SPIN_LOCK_DBG_VFE_CONFIG_SEND_1] = SPIN_LOCK_EXIT;
			goto config_done;
		}
		...
		if (!table_pending->cmd) {
			kfree(table_pending);
			rc = -ENOMEM;
			spin_lock_dbg[SPIN_LOCK_DBG_VFE_CONFIG_SEND_2] = SPIN_LOCK_ENTER;
			spin_unlock_irqrestore(&vfe2x_ctrl->table_lock, flags);
			spin_lock_dbg[SPIN_LOCK_DBG_VFE_CONFIG_SEND_2] = SPIN_LOCK_EXIT;
			goto config_done;
		}
		...
		spin_lock_dbg[SPIN_LOCK_DBG_VFE_CONFIG_SEND_3] = SPIN_LOCK_ENTER;
		spin_unlock_irqrestore(&vfe2x_ctrl->table_lock, flags);
		spin_lock_dbg[SPIN_LOCK_DBG_VFE_CONFIG_SEND_3] = SPIN_LOCK_EXIT;
	} else {
		if (queue == QDSP_TABLEQUEUE) {
			...
			spin_lock_dbg[SPIN_LOCK_DBG_VFE_CONFIG_SEND_4] = SPIN_LOCK_ENTER;
			spin_unlock_irqrestore(&vfe2x_ctrl->table_lock, flags);
			spin_lock_dbg[SPIN_LOCK_DBG_VFE_CONFIG_SEND_4] = SPIN_LOCK_EXIT;
		} else {
			...
			rc = msm_adsp_write(vfe_mod, queue,
				cmd_data, vfecmd.length + 4);
			spin_lock_dbg[SPIN_LOCK_DBG_VFE_CONFIG_SEND_5] = SPIN_LOCK_ENTER;
			spin_unlock_irqrestore(&vfe2x_ctrl->table_lock, flags);
			spin_lock_dbg[SPIN_LOCK_DBG_VFE_CONFIG_SEND_5] = SPIN_LOCK_EXIT;
			CDBG("%x\n", vfecmd.length + 4);
		}
	}
		...
		struct msm_cam_media_controller *mctl)
{
	...
	vfe_recover_flag = 0;
	vfe2x_ctrl->vfe_started = 0;
	memset(spin_lock_dbg, 0, SPIN_LOCK_DBG_MAX);		
...
void msm_vfe_dump_spin_lock_dbg(void)
{
	int i;

	for (i=0; i<SPIN_LOCK_DBG_MAX; i++)
	{
		pr_info("spin_lock_dbg[%d] = %d\n", i, spin_lock_dbg[i]);
	}
	return;
}

void msm_vfe_recover_stop(void)
{
	int rc;
	size_t len;
	void   *cmd_data = NULL;
	...
	*(uint32_t *)cmd_data = VFE_IMMEDIATE_STOP;
	...
	rc = wait_event_timeout(stopevent.wait,stopevent.state==1,1000);
	if(!rc) {
		pr_info("%s:STOP ACK timeout",__func__);
	}
	return;
}
