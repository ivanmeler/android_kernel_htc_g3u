...
inline static int32_t msm_sensor_rawchip_set_size(struct msm_sensor_ctrl_t *s_ctrl, int res)
{
    int rc=0;
#ifdef CONFIG_RAWCHIP
    struct rawchip_sensor_data rawchip_data;
...
static int oem_sensor_init1(void *arg)
{
    struct msm_sensor_ctrl_t *s_ctrl = (struct msm_sensor_ctrl_t *) arg;
    int res;
    struct timespec ts_start, ts_end;
	...
	int mode, int res)
{
	...
	if (s_ctrl->curr_res != res) {
		...
		if (s_ctrl->sensordata->pdata->is_csic || s_ctrl->is_csic ||
			...
			int mode, struct sensor_init_cfg *init_info)
{
	...
	pr_info("s_ctrl->sensordata->pdata->is_csic: %d, s_ctrl->is_csic: %d, s_ctrl->sensordata->csi_if: %d\n",
		s_ctrl->sensordata->pdata->is_csic, s_ctrl->is_csic, s_ctrl->sensordata->csi_if);
...
#if ((defined CONFIG_I2C_CPLD) && ((defined CONFIG_MACH_DUMMY) || (defined CONFIG_MACH_DUMMY) || (defined CONFIG_MACH_DUMMY) || (defined CONFIG_MACH_DUMMY) || (defined CONFIG_MACH_DUMMY) || (defined CONFIG_MACH_DUMMY) || (defined CONFIG_MACH_DUMMY)))
    mdelay(1);
	pr_info("[CAM]%s: sensor reset/pwd high\n", __func__);
    rc = cpld_gpio_write(gpio, 1);
	if(rc < 0){
		pr_err("[CAM]%s: sensor reset/pwd fail\n", __func__);
	}
	mdelay(1);
#else
