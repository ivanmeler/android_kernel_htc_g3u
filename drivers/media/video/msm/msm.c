...
wait_event:
	...
	if (list_empty_careful(&queue->list)) {
		...
		if (rc == -ERESTARTSYS && loop < 20) {
			...
			msm_vfe_dump_spin_lock_dbg();	

			
			if(out->type == MSM_V4L2_STREAM_ON) {
				pr_err("%s: stream on timeout!\n", __func__);
				
				val = readl_relaxed(clockbase);
				pr_err("%sGLBL_CLK_ENA 0xa8600000: %d\n", __func__, val);
				val = readl_relaxed(clockbase + 0x000004ac);
				pr_err("%sGLBL_CLK_ENA_2 0xa86004ac: %d\n", __func__, val);
				val = readl_relaxed(clockbase + 0x00000004);
				pr_err("%sGLBL_CLK_STATE 0xa8600004: %d\n", __func__, val);
				val = readl_relaxed(clockbase + 0x000004b0);
				pr_err("%sGLBL_CLK_STATE_2 0xa86004b0: %d\n", __func__, val);
				val = readl_relaxed(clockbase + 0x00000484);
				pr_err("%sAPPS_CLK_ENA 0xa8600484: %d\n", __func__, val);
				val = readl_relaxed(clockbase + 0x00000044);
				pr_err("%sVFE_NS_REG 0xa8600044: %d\n", __func__, val);
				val = readl_relaxed(clockbase + 0x00000040);
				pr_err("%sVFE_MD_REG 0xa8600040: %d\n", __func__, val);
				val = readl_relaxed(clockbase + 0x0000010C);
				pr_err("%sCLK_ HALT_STATEB 0xa860010c: %d\n", __func__, val);
				val = readl_relaxed(clockbase + 0x000004e0);
				pr_err("%sCLK_HALT_STATEC 0xa86004e0: %d\n", __func__, val);
				val = readl_relaxed(clockbase + 0x000004a4);
				pr_err("%sCSI_NS_REG 0xa86004a4: %d\n", __func__, val);
				pr_err("%s: stream on timeout! Block_IN_VFE_RESET=%d\n", __func__,Block_IN_VFE_RESET);
...
static int msm_open_server(struct file *fp)
{
	...
	clockbase = ioremap(0xa8600000 , 4096 );
