	...
	if (err && cmd->retries && !mmc_card_removed(host->card)) {
		...
		if (mrq->data) {
			if (host->perf_enable) {
				...
				if (host->tp_enable)
					trace_mmc_request_done(mmc_hostname(host),
				    ...
				    struct mmc_async_req *areq, int *error)
{
	...
	if (host->areq) {
		...
		ktime_t wait_diff, wait_ready;
		wait_diff = wait_ready = io_diff;
		...
		if (mmc_card_sd(host->card)) {
			...
			if (ktime_to_us(io_diff) > 400000)
				pr_info("%s (%s), finish cmd(%d) s_sec %d, size %d, ready time = %lld us, total I/O time = %lld us\n",
...
out:
	...
	if (card->host->tp_enable)
		trace_mmc_request_done(mmc_hostname(card->host), MMC_ERASE, from, to - from, ktime_to_ms(diff));
	return err;
}
