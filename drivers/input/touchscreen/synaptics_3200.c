...
static int i2c_syn_error_handler(struct synaptics_ts_data *ts, uint8_t reset, char *reason, const char *fun_name)
{
	...
	#if defined(CONFIG_CPLD)
		else if (ts->cpld_gpio_reset) {
			ret=cpld_gpio_write(ts->cpld_gpio_reset, 0);
			if(ret < 0){
			printk(KERN_ERR "[TP]%s: cpld_gpio_write CPLD_EXT_GPIO_TP_RST 0 fail\n", __func__);
			}
			hr_msleep(1);
			ret=cpld_gpio_write(ts->cpld_gpio_reset, 1);
			if(ret < 0){
			printk(KERN_ERR "[TP]%s: cpld_gpio_write CPLD_EXT_GPIO_TP_RST 1 fail\n", __func__);
			}
			printk(KERN_INFO "[TP] %s: synaptics touch chip reseted.(CPLD)\n", __func__);
		}
	#endif
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (buf[0] == '1' && ts->cpld_gpio_reset) {
		rc=cpld_gpio_write(ts->cpld_gpio_reset, 0);
		if(rc < 0){
		printk(KERN_ERR "[TP]%s: cpld_gpio_write CPLD_EXT_GPIO_TP_RST 0 fail\n", __func__);
		}
		
		hr_msleep(1);
		rc=cpld_gpio_write(ts->cpld_gpio_reset, 1);
		if(rc < 0){
		printk(KERN_ERR "[TP]%s: cpld_gpio_write CPLD_EXT_GPIO_TP_RST 1 fail\n", __func__);
		}
...
static int syn_probe_init(void *arg)
{
	...
	if (pdata) {
		...
		ts->gpio_reset = pdata->gpio_reset;
		#if defined(CONFIG_CPLD)
		ts->cpld_gpio_reset = pdata->cpld_gpio_reset;
		#endif
