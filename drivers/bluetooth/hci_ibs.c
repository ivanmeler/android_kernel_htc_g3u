...
static void __ibs_msm_serial_clock_on(struct tty_struct *tty)
{
	...
	#ifdef CONFIG_QCT2243_V21
	qct_msm_hs_request_clock_on(port);
	#else
...
static void __ibs_msm_serial_clock_request_off(struct tty_struct *tty)
{
	...
	#ifdef CONFIG_QCT2243_V21
	qct_msm_hs_request_clock_off(port);
	#else
...
static void ibs_msm_serial_clock_vote(unsigned long vote, struct hci_uart *hu)
{
	...
	if (new_vote != old_vote) {
		...
		BT_INFO("HCIUART_IBS: vote msm_serial_hs clock %lu(%lu)",
			new_vote, vote);
...
static void hci_ibs_tx_idle_timeout(unsigned long arg)
{
	...
	BT_INFO("hu %p idle timeout in %lu state", hu, ibs->tx_ibs_state);
...
static void hci_ibs_wake_retrans_timeout(unsigned long arg)
{
	...
	BT_INFO("hu %p wake retransmit timeout in %lu state",
		hu, ibs->tx_ibs_state);
