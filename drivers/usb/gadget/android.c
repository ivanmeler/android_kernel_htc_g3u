...
static int serial_driver_initial(struct android_usb_function *f,
					struct usb_composite_dev *cdev)
{
	char *name, *str[2];
...
static __maybe_unused struct android_usb_function mtp_function = {
	.name		= "mtp",
...
void msm_set_android_binding(int is_binding);
...
static void android_usb_init_work(struct work_struct *data)
{
	...
	msleep(100);
	usb_gadget_connect(cdev->gadget);
	msm_set_android_binding(0);
	dev->enabled = true;
	pr_info("%s: ret: %d\n", __func__, ret);
	if (dev->bSwitchFunWhileInit == true) {
		func = htc_usb_get_func_combine_value();
		pr_info("%s: Switch function while init, pre-func = %d, func = %d\n",
				__func__, func, dev->SwitchFunCombination);
		android_switch_function(func | dev->SwitchFunCombination);
