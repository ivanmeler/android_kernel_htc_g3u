...
static void ac_detect_expired(unsigned long _data)
{
	...
	if (dev->connect_type == CONNECT_TYPE_USB || dev->ac_detect_count >= 5)
		return;
...
static void msm_otg_notify_charger_attached(int connect_type)
{
	...
	case CONNECT_TYPE_UNKNOWN:
		...
		if (motg->connect_type != CONNECT_TYPE_USB) {
			...
			mod_timer(&motg->ac_detect_timer, jiffies + (2 * HZ));
		} else {
...
static void msm_otg_start_host(struct otg_transceiver *xceiv, int on)
{
	...
	if (on) {
		...
		USBH_DEBUG("host off\n");
		if (USB_disabled && stop_usb_host == TRY_STOP_HOST_STATE)
...
static int msm_otg_suspend(struct msm_otg *dev)
{
	...
	if (atomic_read(&dev->in_lpm))
		goto out;
	dcp = chg_type == USB_CHG_TYPE__WALLCHARGER;
#ifdef CONFIG_USB_MSM_ACA
	...
	while (!is_phy_clk_disabled()) {
		if (time_after(jiffies, timeout)) {
			if (!dcp)
				...
				USBH_WARNING("%s: Unable to suspend PHY, chg_type = %d\n", __func__, dcp);
			spin_lock_irqsave(&dev->lock, flags);
...
void msm_otg_set_disable_usb(int disable_usb)
{
	...
	if(!disable_usb && dev->otg.state == OTG_STATE_B_PERIPHERAL)
		fromUSB_disabled=1;
	queue_work(dev->wq, &dev->sm_work);
}
...
static void msm_otg_sm_work(struct work_struct *w)
{
	...
	case OTG_STATE_B_PERIPHERAL:
		if (!test_bit(ID, &dev->inputs) ||
			...
			otg_pm_qos_update_latency(dev, 0);
		} else if ((dev->connect_type == CONNECT_TYPE_USB && USB_disabled) || fromUSB_disabled ) {
