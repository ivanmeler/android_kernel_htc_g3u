...
extern char swap_part[64];
sector_t swap_read_count = 0;
sector_t swap_write_count = 0;
sector_t other_read_count = 0;
sector_t other_write_count = 0;
ktime_t mmc_swap_t;
...
#ifdef CONFIG_MMC_PERF_PROFILING
		if (!board_mfg_mode() && !get_tamper_sf() && !strncmp(swap_part, bdevname(bio->bi_bdev, tmp), strlen(swap_part))) {
			if ((swap_read_count == 0) && (swap_write_count == 0) && (other_write_count == 0) && (other_write_count == 0))
				mmc_swap_t = ktime_get();

			if (rw & WRITE)
				swap_write_count += (count>>1);
			else
				swap_read_count += (count>>1);

			diff_swap_t = ktime_sub(ktime_get(), mmc_swap_t);
			if (ktime_to_ms(diff_swap_t) >= 3000LL) {
				printk(KERN_INFO "mmc0 Statistics: (swap) read %Lu KB write %Lu KB, (others) read %Lu KB write %Lu KB (%lld ms)\n", swap_read_count, swap_write_count
			, other_read_count, other_write_count, ktime_to_ms(diff_swap_t));
				swap_write_count = 0;
				swap_read_count = 0;
				other_write_count = 0;
				other_read_count = 0;
			}
		} else if (!board_mfg_mode() && !get_tamper_sf()) {
			if ((swap_read_count == 0) && (swap_write_count == 0) && (other_write_count == 0) && (other_write_count == 0))
				mmc_swap_t = ktime_get();

			if (rw & WRITE)
				other_write_count += (count>>1);
			else
				other_read_count += (count>>1);
