...
static inline void hci_auth_complete_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if (conn) {
		if (ev->status == 0x06 && hdev->ssp_mode > 0 &&
			...
			if (key->key_type == 0x05) {
				BT_ERR("hci_auth_complete_evt: Previous key was authenticated, "
					"updating the auth type for outgoing conenction");
...
static inline void hci_io_capa_request_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if (test_bit(HCI_PAIRABLE, &hdev->flags) ||
		...
		bacpy(&cp.bdaddr, &ev->bdaddr);
		BT_ERR("hci_io_capa_request_evt: conn->auth_type = %d", conn->auth_type);
		if (conn->auth_initiator)
